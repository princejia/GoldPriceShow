...
「每日走势」走 goldapi 的历史接口，按日期逐天取；历史数据不会再变，成功过的日期
会一直复用，只有跨到新的交易日才会多一次请求。为了不让冷启动一次把额度打光，单次渲染
最多现取 2 天，其余等后续请求慢慢补齐。取数失败（常见是配额用尽）时该模块静默隐藏，
403/429 冷却 6 小时，其他错误冷却 15 分钟。
